...
export class HttpClientService {
  ...
  async getUserProfile(userId: string): Promise<UserProfile | null> {
    try {
      const response = await this.authServiceClient.get(`/auth/users/${userId}`);
  ...
  async getWorkerProfile(workerId: string): Promise<WorkerProfile | null> {
    try {
      const response = await this.workServiceClient.get(`/work/workers/${workerId}`);
      return response.data.data || response.data;
    } catch (error: any) {
  ...
  async getUserProfiles(userIds: string[]): Promise<UserProfile[]> {
    try {
      const response = await this.authServiceClient.post('/auth/users/batch', { userIds });
      return response.data.data || response.data;
    } catch (error: any) {
  ...
  async getWorkerProfiles(workerIds: string[]): Promise<WorkerProfile[]> {
    try {
      const response = await this.workServiceClient.post('/work/workers/batch', { workerIds });
      return response.data.data || response.data;
    } catch (error: any) {
